...
public class Crawler extends WebCrawler {
     ...
     * the crawling logic. Here URLs with extensions css, js, images etc will not be visited
     */
    @Override
    ...
    public void visit(Page page) {
        ...
        if (DOC_PATTERN.matcher(url).matches()) {
            ...
            // .txt files
            if (page.getParseData() instanceof TextParseData txtParseData) {
                try {
...
//                	ErrorLogger.logException(e, fileName, SITE_ID);
    ...
    public static String generateNameFromUrl(String url) {
        ...
        uniqueName = uniqueName.substring(0, uniqueName.lastIndexOf('_'))
                + "." + uniqueName.substring(uniqueName.lastIndexOf('_') + 1);
        return uniqueName;
    }
